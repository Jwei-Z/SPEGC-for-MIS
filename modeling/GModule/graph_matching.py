...
from modeling.GModule.utils.losses import BCEFocalLoss
from modeling.GModule.utils.affinity import Affinity
from modeling.GModule.utils.attentions import MultiHeadAttention, HyperGraph
from modeling.GModule.utils.graph_network import MAGNN, Topo_Graph
from modeling.GModule.utils.sinkhorn_distance import SinkhornDistance
from modeling.GModule.utils.adaptation_loss import ProtoLoss
...
class GModule(torch.nn.Module):
    """
     - CTTA
    Graph Matching Module - supports CTTA (Continual Test-Time Adaptation)

    : 
    Training: Uses original multi-graph matching logic

    : , 
    Testing: Single image stream input, builds dynamic batch via graph pool

    CTTA (CTTA Stream Processing Features):
    - graph_pool: , , , 
    - update_graph_pool(): FIFO
    - create_pseudo_batch(): 
    - _forward_ctta_stream(): CTTA
    - _process_pseudo_batch(): 
    """
    ...
    def __init__(self, opt, in_channels):
        ...
        # CTTA - Graph Pool for CTTA stream processing
        self.graph_pool_size = opt.TEST.CTTA_POOL_SIZE
        self.ctta_min_pool_size = opt.TEST.CTTA_MIN_POOL_SIZE
        self.ctta_enable_single_graph = opt.TEST.CTTA_ENABLE_SINGLE_GRAPH
        self.graph_pool = []  # (nodes, labels, weights)
        # print(opt.MODEL)
    ...
    def update_graph_pool(self, nodes, labels, weights):
        """
        , FIFO
        Update graph pool using FIFO strategy
        ...
        Args:
            ...
            weights: 
        """
        if len(self.graph_pool) >= self.graph_pool_size:
            self.graph_pool.pop(0)
        self.graph_pool.append((nodes.clone(), labels, weights))

    def create_pseudo_batch(self, current_nodes, current_labels, current_weights):
        """
        CTTA
        Create dynamic graph batch for CTTA stream processing
        ...
        Returns:
            tuple: (nodes_batch, labels_batch, weights_batch) 
        """
        if len(self.graph_pool) == 0:
            # :
            return ([current_nodes], [current_labels], [current_weights])
        elif len(self.graph_pool) < self.graph_pool_size:
            # :  +
            pool_nodes = [item[0] for item in self.graph_pool]
            pool_labels = [item[1] for item in self.graph_pool]
            pool_weights = [item[2] for item in self.graph_pool]

            nodes_batch = [current_nodes] + pool_nodes
            labels_batch = [current_labels] + pool_labels
            weights_batch = [current_weights] + pool_weights

            return (nodes_batch, labels_batch, weights_batch)
        else:
            # :  + N
            recent_graphs = self.graph_pool[-self.graph_pool_size:]
            pool_nodes = [item[0] for item in recent_graphs]
            pool_labels = [item[1] for item in recent_graphs]
            pool_weights = [item[2] for item in recent_graphs]
    ...
    def forward(self, images, features, targets=None, score_maps=None):
        '''
         - CTTA
        Graph matching forward pass - supports training and CTTA test modes
        ...
        if self.training:
            # :
            features, feat_loss = self._forward_train_original(images, features, targets, score_maps)
            return features, feat_loss
        else:
            # : CTTA
            if targets is not None:
                # targetsTTT
                features, feat_loss = self._forward_ctta_stream(images, features, targets, score_maps)
                return features, feat_loss
            else:
    ...
    def _forward_train_original(self, images, features, targets=None, score_maps=None):
        """
         - 
        Original training mode forward pass - keep unchanged
        """
    ...
    def _forward_ctta_stream(self, images, features, targets=None, score_maps=None):
        """
        CTTA: , 
        CTTA stream processing: single image input, graph pool builds dynamic batch
        ...
        Returns:
            tuple: (features, middle_head_loss)
        """

        targets_current = [x['instances'] for x in targets] if targets else None


        if self.head_in_cfg != 'LN':
            features_current = self.head_in(features)
            nodes_current, labels_current, weights_current = self.graph_generator(
                self.compute_locations(features_current), features_current, targets_current
            )
        else:
            nodes_current = self.head_in_ln(features)
            # LN
            labels_current = torch.zeros(nodes_current.size(0), dtype=torch.long, device=nodes_current.device)
            weights_current = torch.ones(nodes_current.size(0), device=nodes_current.device)


        nodes_batch, labels_batch, weights_batch = self.create_pseudo_batch(
            nodes_current, labels_current, weights_current
        )


        self.update_graph_pool(nodes_current, labels_current, weights_current)


        middle_head_loss = self._process_pseudo_batch(nodes_batch, labels_batch, weights_batch)

        return features, middle_head_loss

    def _process_pseudo_batch(self, nodes_batch, labels_batch, weights_batch):
        """
        CTTA
        Process dynamic graph batch for CTTA stream matching
        ...
        for nodes in nodes_batch:
            if self.with_complete_graph:
                proc_nodes, proc_edges = self._forward_intra_domain_graph(nodes)
                processed_nodes.append(proc_nodes)
                processed_edges.append(proc_edges)
            else:
                processed_nodes.append(nodes)
                processed_edges.append(None)


        if num_graphs == 1:
            # : ,
            if self.ctta_enable_single_graph:
                nodes = processed_nodes[0]
                labels = labels_batch[0]
                weights = weights_batch[0]

                node_loss = self._forward_node_loss(nodes, labels, weights)
                middle_head_loss.update({'loss_node': self.weight_nodes * node_loss})

        elif num_graphs >= 2 and len(self.graph_pool) >= self.ctta_min_pool_size:
            # :
            nodes_1, nodes_2 = processed_nodes[0], processed_nodes[1]
            labels_1, labels_2 = labels_batch[0], labels_batch[1]
            weights_1, weights_2 = weights_batch[0], weights_batch[1]
            ...
            if self.matching_cfg != 'none':
                ...
                if self.with_quadratic_matching and processed_edges[0] is not None and processed_edges[1] is not None:
                    matching_loss_quadratic = self._forward_qu(nodes_1, nodes_2, processed_edges[0].detach(), processed_edges[1].detach(), affinity)
                    middle_head_loss.update({'loss_mat_qu': matching_loss_quadratic})
