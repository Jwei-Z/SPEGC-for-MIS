...
class PrototypeComputation(object):
    ...
    def prepare_targets(self, points, targets):
        ...
        # Originally all labels are concating in a batch, now separated
        # labels_level_first = []
    ...
    def __call__(self, features, targets, uncertainty_maps=None, spegc_p=0.5):
            ...
            for b in range(N):
                ...
                for l, label in enumerate(labels[b]):  
                    ...
                    if uncertainty_maps is not None:
                        u_map = uncertainty_maps[l][b].reshape(-1)
                        pos_u = u_map[pos_indx]
                        num_pos = len(pos_u)
                        num_keep = max(1, int(num_pos * spegc_p))
                        sorted_idx = torch.argsort(pos_u)[:num_keep]
                        pos_nodes_all = pos_nodes_all[sorted_idx]
                        pos_labels_all = pos_labels_all[sorted_idx]
